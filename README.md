...
- 针对缺少 Cholec80 原始数据的场景提供结构检查和轻量测试，便于 GitHub 上稳定展示。

## 复现边界

- 仓库不分发 Cholec80 数据集；训练和完整评估需要用户按 README 准备本地数据。
- `scripts/check_project.sh` 与 `pytest tests/ -q` 不依赖私有数据，可用于快速检查代码健康状态。
- `picture/` 中保留训练流程图和对比图，帮助在没有数据集的情况下理解方法和结果组织。
